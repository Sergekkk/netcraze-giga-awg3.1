...
3. **Обеспечить отказоустойчивость**: автоматический rollback при сбоях, фоновый watchdog для контроля handshake каналов, команда точечного восстановления `repair` и постоянное логирование.
4. **Управлять маршрутизацией** через веб-интерфейс Keenetic (Политики подключений, Multi-WAN, приоритеты, DNS).
...
└── router/                    # Файлы и скрипты для установки на роутер
    ...
    │   ├── etc/
    │   │   ├── init.d/
    │   │   │   ├── S99awg3         # Сервисный SysV init скрипт (start/stop/repair)
    │   │   │   └── S100awg3-watchdog # Фоновый watchdog для контроля handshake
    │   │   └── awg3/
    │   │       └── conf/           # Папка для конфигов (и подпапка parsed/)
    │   │           └── awg3_example.conf # Пример конфига с параметрами AWG3
    │   └── var/
    │       └── log/                # Директория логов (awg3.log, awg3-watchdog.log)
```
...
### 4. Запуск служб
Запустите основной сервис туннелей и сторожевой таймер (watchdog):
```bash
# Запуск основного сервиса AWG3
/opt/etc/init.d/S99awg3 start

# Запуск фонового watchdog (контроль handshake и автовосстановление)
/opt/etc/init.d/S100awg3-watchdog start
```

### 5. Проверка статуса, логов и управление
```bash
# Статус службы и список поднятых туннелей
/opt/etc/init.d/S99awg3 status

# Статус службы мониторинга watchdog
/opt/etc/init.d/S100awg3-watchdog status

# Прямой вывод данных об активности пиров, handshake и трафике
/opt/bin/awg show

# Просмотр системных логов с отметками времени
tail -n 50 /opt/var/log/awg3.log
tail -n 50 /opt/var/log/awg3-watchdog.log

# Ручной запуск проверки всех каналов через watchdog
/opt/etc/init.d/S100awg3-watchdog check

# Точечный перезапуск/восстановление конкретного туннеля (по индексу, например 0)
/opt/etc/init.d/S99awg3 repair 0
